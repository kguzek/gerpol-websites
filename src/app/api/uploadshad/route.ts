...
import { FilePayloadSchema, S3Service } from "@/components/uploadshad/utils";
...
export async function POST(request: NextRequest) {
  ...
  if (signedURLResult.failure) {
    ...
    return new NextResponse(
      JSON.stringify({ message: "Failed to generate signed URL." }),
      {
        status: 500,
        headers: {
...
export async function DELETE(request: NextRequest) {
  ...
  if (!url)
    return NextResponse.json({ message: "Missing query values" }, { status: 400 });
