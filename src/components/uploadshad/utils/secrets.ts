...
const clientVariablesSchema = z.object({ ...clientSchema });

const clientVariablesWithS3Schema = z.object({
  ...clientSchema,
  NEXT_PUBLIC_AWS_BASE_URL: z.string().nonempty(),
});

const clientVariables = {
  NEXT_PUBLIC_HOST_URL: process.env.NEXT_PUBLIC_HOST_URL ?? WEBSITE_URL,
...
export function getClientVariablesWithS3() {
  if (!clientVariables.NEXT_PUBLIC_HOST_URL) {
    throw new Error("UploadShad Error: Missing NEXT_PUBLIC_HOST_URL");
  ...
  const safeClientVariables = clientVariablesWithS3Schema.safeParse(clientVariables);
  if (!safeClientVariables.success) {
    throw new Error("UploadShad Error: Invalid NEXT_PUBLIC env variables");
  }
...
export function getServerVariables() {
  if (!AWSVariables.AWS_BUCKET_NAME) {
    throw new Error(
      "UploadShad Error: Missing AWS env variable. Ensure AWS_BUCKET_NAME is set",
    );
  } else if (!AWSVariables.AWS_BUCKET_REGION) {
    throw new Error(
      "UploadShad Error: Missing AWS env variable. Ensure AWS_BUCKET_REGION is set",
    );
